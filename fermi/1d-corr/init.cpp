...
void Init_dip(DIPOLE &dipole, DEOM *d, const Json &json) {
  dipole.sdip = nNNmat(d->nmod);
  for (int k = 0; k < d->nmod; k++)
  ...
  dipole.pdipa = nNNmat(d->nmod);
  for (int k = 0; k < d->nmod; k++)
    for (int i = 0; i < NSYS; i++)
      for (int j = 0; j < NSYS; j++)
        dipole.pdipa[k](i, j) = json["pdipa_cub"]["real"].array_items()[INDEX3(k, i, j)].number_value() + complex<double>(1i) * json["pdipa_cub"]["imag"].array_items()[INDEX3(k, i, j)].number_value();

  dipole.pdipc = nNNmat(d->nmod);
  for (int k = 0; k < d->nmod; k++)
    for (int i = 0; i < NSYS; i++)
      for (int j = 0; j < NSYS; j++)
        dipole.pdipc[k](i, j) = json["pdipc_cub"]["real"].array_items()[INDEX3(k, i, j)].number_value() + complex<double>(1i) * json["pdipc_cub"]["imag"].array_items()[INDEX3(k, i, j)].number_value();
...
void Init_sys(DEOM *d, const Json &json, const char *input_name) {
  ...
  d->nmodmaxLabel = VectorXi(d->nind);
  for (int i = 0; i < d->nind; i++)
    d->nmodmaxLabel(i) = json["nmodmaxLabel"]["real"].array_items()[i].int_value();

  d->delt_res = VectorXcd::Zero(d->nmod);
  ...
  d->qmd1a = nNNmat(d->nmod);
  for (int i = 0; i < d->nmod; i++)
    for (int j = 0; j < NSYS; j++)
      for (int k = 0; k < NSYS; k++)
        d->qmd1a[i](j, k) = json["qmd1a"]["real"].array_items()[INDEX3(i, j, k)].number_value() + complex<double>(1i) * json["qmd1a"]["imag"].array_items()[INDEX3(i, j, k)].number_value();
  d->qmd1c = nNNmat(d->nmod);
  for (int i = 0; i < d->nmod; i++)
    for (int j = 0; j < NSYS; j++)
      for (int k = 0; k < NSYS; k++)
        d->qmd1c[i](j, k) = json["qmd1c"]["real"].array_items()[INDEX3(i, j, k)].number_value() + complex<double>(1i) * json["qmd1c"]["imag"].array_items()[INDEX3(i, j, k)].number_value();
  ...
  // ##  init keys and gams ##
  d->keys = XXbmat_r(d->nmax, d->nind);
  for (int iado = 0; iado < d->nmax; iado++) {
    for (int mp = 0; mp < d->nind; mp++) {
      d->keys(iado, mp) = false;
    }
  }

  d->emptykey = XXbvet_r(d->nind);
  for (int i = 0; i < d->nind; i++) {
    d->emptykey[i] = false;
  }

  d->zerokey = XXbvet_r(d->nind);
  for (int i = 0; i < d->nind; i++) {
    d->zerokey[i] = false;
  }
  ...
  d->qmdta = nNNmat(d->nind);
  d->qmdtc = nNNmat(d->nind);
...
void Init_aux(DEOMAUX *daux, DEOM *d, const Json &json) {
  daux->keys1 = XXbmat_r(d->nmax, d->nind);

  daux->key_tmp = XXbvet_r(d->nind);
