...
# for vib_a, S_0, npsd, lmax in itertools.product([0, 0.5, 1.0], [0.25, 0.5], [1, 2], [40, 50, 60]):
# for vib_a, S_0, (npsd, lmax), dt in itertools.product([0, 1],
#                                                       [0.1, 0.2, 0.3, 0.4],
#                                                       [(1, 40),
#                                                        (2, 50)], [0.0005]):
for vib_a, S_0, (npsd,
                 lmax), ampl, dt in itertools.product([1, 0.5, 0],
                                                      [0.4, 0.3, 0.2, 0.1],
                                                      [(2, 35)],
                                                      [0.1, 0.5], [0.0005]):
    # for vib_a, S_0, npsd, lmax in itertools.product([0, 1, 2, 3], [0.1, 0.2, 0.3, 0.4], [1, 2], [40, 50]):
    nmod = 2
    omgs = 1.0  # in 200cm-1
    gams = 0.1  # in 20 cm-1
    # Deltesip = 100  # in 20000cm-1
    Deltesip = 50  # in 10000cm-1
    temp = 77 * 0.69503457 / 200  # in 77K
    beta = 1 / temp
    lamdxx = 0.1  # in 10000cm-1
    D2 = 2.0 * S_0
    # etas = omgs * 2.0
    # lams = D2 / 2.0
    # lamd0 = omgs * D2 / 2.0
    ...
    spe_vib_sp = D_sp * D_sp * omgs_sp * omgs_sp * xqq_sp
    spe_vib_b_sp = 0
    spe_b_sp = 0.5 * phiyy_sp
    spe_sp = 0
    ...
    arma_write(
        etar, 'inp_etar_{}_{}_{}_{}.mat'.format(str(S_0), str(vib_a),
                                                str(lmax), str(npsd)))
    arma_write(
        etal, 'inp_etal_{}_{}_{}_{}.mat'.format(str(S_0), str(vib_a),
                                                str(lmax), str(npsd)))
    arma_write(
        etaa, 'inp_etaa_{}_{}_{}_{}.mat'.format(str(S_0), str(vib_a),
                                                str(lmax), str(npsd)))
    arma_write(
        mode, 'inp_mode_{}_{}_{}_{}.mat'.format(str(S_0), str(vib_a),
                                                str(lmax), str(npsd)))
    arma_write(
        expn, 'inp_expn_{}_{}_{}_{}.mat'.format(str(S_0), str(vib_a),
                                                str(lmax), str(npsd)))
    ...
    hams[1, 1] = Deltesip + lamd

    hame = np.zeros((2, 2), dtype=complex)
    hame[1, 1] = Deltesip + lamd
    ...
    sdip[0, 0, 1] = sdip[0, 1, 0] = 1.0
    arma_write(sdip, 'inp_sdip.mat')

    pdip = np.zeros((nmod, 2, 2), dtype=float)
    pdip[0, 0, 1] = pdip[0, 1, 0] = 1.0 * vib_a
    # pdip[1, 0, 1] = pdip[1, 1, 0] = 1.0 * vib_a
    arma_write(pdip, 'inp_pdip.mat')

    bdip = 1.0 * np.ones(12 + 2 * npsd, dtype=float)
    ...
    jsonInit = {
        ...
        "ampl": ampl,
        "freq": np.real(50.2),
        "sigm": 4 / 26.54594499255,
        "varp": 0.0
        #  convert:
        #  1.0545718 × 10-34 j s
        #  1.0545718 × 10-34 j fs 5.03445e22 cm-1/j 1e15 fs/s
        #  5309.188998510001cm-1 fs
        #  1 200cm-1 26.54594499255fs HERE
        #  1 ENE = 200cm-1
        #  1 TME = 26.54594499255fs
        #  2 * np.pi / np.real(50.2) is 3.322571544669077
    ...
    with open('input.json', 'w') as f:
        json.dump(jsonInit, f, indent=8, default=convert)

    cmd = r'export OMP_NUM_THREADS=7' + '&&'
    cmd += r'/usr/local/bin/jemalloc.sh  ../a.out' + '&&'
    cmd += r'mv input.json input.json' + \
        '_' + str(S_0) + '_' + str(vib_a) + \
        '_' + str(lmax) + '_' + str(npsd) + '_' + str(ampl) + '_' + str(dt) + '&&'
    cmd += r'mv prop-pol.dat prop-pol.dat' + \
        '_' + str(S_0) + '_' + str(vib_a) + \
        '_' + str(lmax) + '_' + str(npsd) + '_' + str(ampl) + '_' + str(dt)
    os.system(cmd)
