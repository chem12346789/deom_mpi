...
void rem_oprt(nNNmat &ddos1, const nNNmat &total, const DEOM *d, const int iado, const XXimat_r &keys, const XXimat_r &g_index_list_pos, const char lcr) {
  int pos;
  MatrixXcd ddos;
  ddos = MatrixXcd::Zero(NSYS, NSYS);
  ...
  for (int mp = 0; mp < d->nind; mp++) {
    ...
    if (pos != -1) {
      ...
      for (int ii = 0; ii < d->nmodmax; ii++) {
        if (lcr == 'l') {
          ddos.noalias() += d->dipole.bdip[mp] * sqrt(n) / d->coef_abs(mp) * (d->coef_lft[mp](m, ii) * d->dipole.pdip[ii] * total[pos]);
        } else if (lcr == 'r') {
          ddos.noalias() += d->dipole.bdip[mp] * sqrt(n) / d->coef_abs(mp) * (d->coef_rht[mp](m, ii) * total[pos] * d->dipole.pdip[ii]);
        } else if (lcr == 'c') {
          ddos.noalias() += d->dipole.bdip[mp] * sqrt(n) / d->coef_abs(mp) * (d->coef_lft[mp](m, ii) * d->dipole.pdip[ii] * total[pos] - d->coef_rht[mp](m, ii) * total[pos] * d->dipole.pdip[ii]);
        }
      }
    }
    ...
    if (pos != -1) {
      for (int ii = 0; ii < d->nmodmax; ii++) {
        if (lcr == 'l') {
          ddos.noalias() += d->dipole.bdip[mp] * sqrt(n + 1) * d->coef_abs(mp) * d->dipole.pdip[ii] * total[pos];
        } else if (lcr == 'r') {
          ddos.noalias() += d->dipole.bdip[mp] * sqrt(n + 1) * d->coef_abs(mp) * total[pos] * d->dipole.pdip[ii];
        } else if (lcr == 'c') {
          ddos.noalias() += d->dipole.bdip[mp] * sqrt(n + 1) * d->coef_abs(mp) * (d->dipole.pdip[ii] * total[pos] - total[pos] * d->dipole.pdip[ii]);
        }
      }
    }
  }
