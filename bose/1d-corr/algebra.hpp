...
// This part of the code is for testing. You should consider using it if you do not know whether your code works.
...
// this part of code is for performance/product, having some function as gen_key(key,pos,1)
llint gen_key_p(const XXuivet_r &key0, XXuivet_r &key1, const int pos, const int chg, const DEOM *d) {
  ullint hash_value = 1;
  key1 = key0;
  key1(pos) += chg;
  int sum = 0;
...
// this part of code is for performance/product, having some function as gen_key(key,pos,-1)
// and not that there is no positive change here(NOT numis something positive IS add something negative)
llint gen_key_n(const XXuivet_r &key0, XXuivet_r &key1, const int pos, const int chg, const DEOM *d) {
  ullint hash_value = 1;
  key1 = key0;
  key1(pos) += chg;
  int sum = 0;
