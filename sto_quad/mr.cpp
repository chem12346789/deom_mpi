...
void rem_cal(nNNmat &ddos1, const nNNmat &total, const DEOM *d, const int iado, const XXimat_r &keys, const XXimat_r &g_index_list_pos) {
  ...
  for (int mp = 0; mp < d->nind; mp++) {
    ...
    if (pos != -1) {
      int m = d->modLabel(mp);
      // ddos.noalias() -= (complex<double>)1i * sqrt(n) / d->coef_abs(mp) * (d->coef_lft[mp](m, 0) * d->qmdl[mp] * total[pos] - d->coef_rht[mp](m, 0) * total[pos] * d->qmdr[mp]);
      for (int ii = 0; ii < d->nmodmax; ii++) {
        ddos.noalias() -= (complex<double>)1i * sqrt(n) / d->coef_abs(mp) * (d->coef_lft[mp](m, ii) * d->qmdl[mp] * total[pos] - d->coef_rht[mp](m, ii) * total[pos] * d->qmdr[mp]);
      }
    }

    pos = g_index_list_pos(iado, mp + 1 + d->nind) - 1;
    if (pos != -1) {
      ddos.noalias() -= (complex<double>)1i * sqrt(n + 1) * d->coef_abs(mp) * (d->qmdl[mp] * total[pos] - total[pos] * d->qmdr[mp]);
    }
  }
...
#pragma omp critical
    ...
    printf("Maximum l reached!!Maybe unsafed\n");
  }
...
#pragma omp critical
      ...
      printf("Maximum l reached!!Maybe unsafe\n");
    }
